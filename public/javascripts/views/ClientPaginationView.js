define([
	...
	return BaseView.extend({
		...
		initialize: function(attrs){
			...
			this.eventBus.on("collection:sliced", this.paginate, this);
		},
		events:{
			"click a" : "getNextPage",
		...
		changeItemsPerPage: function(e){
			...
			serviceManager.loadData(this.collection, {
				action : 'perpage-update',
				currentPageNum : 1,
				perPage : $target.val()
			});
		...
		createNav: function(){
			...
			for(;i <= this.totalPages; i++){
				...
				if(i === this.options.startRange){
					navhtml += "<span id = 'prev3'> <a href = '#' class = 'breaks'>...</a></span>";
				}

				else if( i === this.totalPages - this.options.endRange){
					navhtml+="<span id = 'next3'> <a href = '#' class = 'breaks'>...</a></span>";
				}
			}
		...
		cacheNavElements: function(){
			...
			this.nav.pages = this.holder.find("a").not(".jp-next, .jp-previous, .jp-first, .jp-last, .breaks");
		...
		getNextPage: function(e){
			var $target = $(e.target),
				...
				if(this.validNewPage(newPage)){
					...
					serviceManager.sliceCollection(this.collection,{
		...
		getNewPageNum: function(nav, target){
			...
			if( parentSpan.length ){
				if( parentSpan[0].id == nav.lastBreak[0].id ){
					return nav.pages.index(nav.currentPage)+4;
				}
				else if(parentSpan[0].id == nav.firstBreak[0].id){
